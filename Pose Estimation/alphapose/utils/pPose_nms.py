...
def write_json(all_results, outputpath, form=None, for_eval=False, outputfile='alphapose-results.json'):
    ...
    #HACK TO FORCE NORMAL POSE
    form = 'normal'
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath, outputfile), writemode) as json_file:
        ...
        if(writemode == 'w'):
            json_results = [json_results] #separation hack
        with open(os.path.join(outputpath, outputfile), writemode) as json_file:
            if(writemode == 'r+'):
                print('updating...')
                file_data = json.load(json_file)
                file_data.append(json_results)
                json_file.seek(0)
