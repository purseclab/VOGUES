...
if __name__ == "__main__":
    ...
    try:
        for i in im_names_desc:
            start_time = getTime()
            with torch.no_grad():
                inputs_pose = (0, 1000) #Todo: Statistical distribution?
                ...
                if inps is not None:
                    try:
                        print("Attacking...")
                        #Let's create a new attack tensor
                        box_targets_arr = torch.clone(cropped_boxes).cpu().detach().numpy()[0]
#                        box_targets_arr[0] = box_targets_arr[0] + 100
#                        box_targets_arr[2] = box_targets_arr[0] + 100
                        
                        
                        attack_targets = torch.Tensor(box_targets_arr.tolist())
#                        attack_inputs = torch.Tensor(torch.clone(cropped_boxes).cpu().detach().numpy()[0].tolist())
#                        print("attack inputs")
#                        print(attack_inputs.size())
#                        print("boxes")
#                        print(cropped_boxes.size())
#                        print("original raw inputs size")
#                        print(inps.size())
#                        print("inputs")
#                        print(inps)
                        print("IS INP VARIABLE")
                        print(isinstance(inps.data, Variable))
                        print("IS ATTACK_TARGETS VARIABLE")
                        print(isinstance(attack_targets.data, Variable))
                        print("IS POSE_MODEL VARIABLE")
                        print(isinstance(pose_model, Variable))

                        adversarial_examples = adversary(pose_model, inps.data, attack_targets.data, to_numpy=False)
                        #ADVERSARIAL TENSOR
                        inps = adversarial_examples #Todo: valid tensor check?
